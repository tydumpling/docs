...
let xj = createByObject(hd, "tydumpling", 12);
console.log(xj);
```
...
```js
let a = { url: "tydumpling" };
let b = { name: "tydumpling" };
Object.setPrototypeOf(a, b);
...
let hd = new User("tydumpling");
let xj = new hd.constructor("tydumpling");
console.log(xj);
```
...
extend(Admin, User);
let hd = new Admin("tydumpling", 19);
hd.show();
hd.total(); //统计积分
hd.ajax(); //请求后台
```
